...
def animation_sequence(sequence,base_path,size=None,colorkey=(255,255,255),transparency=255):
    global animation_database
    result = []
    for frame in sequence:
        image_id = base_path + base_path.split('/')[-2] + '_' + str(frame[0])
        image = pygame.image.load(image_id + '.png').convert()
        if size != None: image = pygame.transform.scale(image, size)
...
def load_animations(path):
    ...
    for animation in data.split('\n'):
        ...
        try:
            size = (int(sections[3]), int(sections[4]))
        except: size = None
        sequence = []
        n = 0
        for timing in timings:
            ...
            n += 1
        anim = animation_sequence(sequence,path + anim_path,size,e_colorkey)
